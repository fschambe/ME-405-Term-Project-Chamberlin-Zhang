...
bumper = Pin('PC8', Pin.IN, Pin.PULL_DOWN)
...
startFlag = Share("B", name="Start Course Flag")
startFlag.put(False)

# delta velocity, adjustments towards speeds in order to steer a certain side
steer = Share("f", name="Steering dv (mm/s)")
steer.put(0.0)
driveEnable = Share("B", name="Drive Enable")
driveEnable.put(False)
...
LINE_PINS.reverse() # pins were reversed in order to fit bump sensor
...
leftMotorTask  = task_motor(left_motor, left_encoder,
                            ...
                            followFlag=followFlag, steer=steer, side=-1, driveEnable=driveEnable)
...
task_list.append(Task(lineTask.run, name="Line Follow Task",
                      priority=2, period=12, profile=True))
