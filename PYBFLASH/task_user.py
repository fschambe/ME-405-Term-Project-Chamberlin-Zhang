...
|* Press Button 2 (Blue Button) to start course                               *|\r
+---+--------------------------------------------------------------------------+\r\n"""
...
class task_user:
    ...
    def __init__(self, leftMotorGo, rightMotorGo, setpoint, Kp, Ki, dataValues, timeValues, dataValues_right=None, timeValues_right=None, sensor=None, followFlag: Share=None, s=None, psi=None):
        ...
        Args:
            ...
            s (Share):            A share object representing the current distance traveled (mm)
            psi (share):          A share object represemting the current heading (rad)
        '''
        ...
        self._dataValues_left: Queue = dataValues # Queue object to store collected encoder position values
        self._timeValues_left: Queue = timeValues # Queue object to store time stamps of datavalues
        self._s = s                               # Share object containing distance traveled
        self._psi = psi                           # Share object containing heading

        # If both motors are being sampled, use two queues to hold for each motor, else, only use 1 queue
