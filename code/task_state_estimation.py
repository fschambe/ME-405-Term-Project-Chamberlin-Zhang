...
where:
    uhat_k = [u_k; y_k]  (stack your inputs and measured outputs)

This file is written to match your existing generator-task style:
- class with run() generator
- uses micropython.const for states
- uses task_share.Share for publishing states
"""
...
class task_state_estimation:
    ...
    def __init__(self,
                 ...
                 right_encoder,
                 imu=None,
                 # inputs u_k (choose what YOUR model uses)
                 u0: Share = None,
                 u1: Share = None,
                 ...
                 xhat2: Share = None,
                 xhat3: Share = None,
                 # optional: publish measured y too
                 y0: Share = None,
                 y1: Share = None,
                 y2: Share = None,
                 y3: Share = None,
        ...
        # self.Bd = np.array([
        #     [0.0311, 0.0311, 0.2449, 0.2449, 0.0, 0.0],
        #     [0.0, 0.0, -0.007, 0.0070, 0.0001, 0.0039],
        #     [0.1278, 0.0805, 0.0882, 0.0882, 0.0, -1.9630],
        #     [0.0805, 0.1278, 0.0882, 0.0882, 0.0, 1.9630],
        # ])
    ...
    def run(self):
        ...
        while True:
            if self._state == S0_INIT:
                ...
                # time bookkeeping (not required for discrete observer update,
                # but useful if you want to sanity-check scheduling)
                t_now = ticks_ms()
                _dt_ms = ticks_diff(t_now, t_prev)
                t_prev = t_now

                # ---------
                # 1) read measured outputs y_k  (EDIT TO MATCH YOUR MODEL)
                # ---------
                ...
                # IMU: you must provide an object with yaw() and yaw_rate() (or similar)
                yaw = 0.0
                yaw_rate = 0.0
                if self._imu is not None:
                    try:
                        yaw = math.radians(self._imu.yaw()-self._psi0)          # degrees or rad (be consistent with your matrices)
                        if yaw > math.pi:
                            yaw -= 2*math.pi
                        elif yaw < -math.pi:
                            yaw += 2*math.pi
                        yaw_rate = math.radians(self._imu.yaw_rate())# deg/s or rad/s
                    except:
                        pass
