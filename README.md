...
The goal of the project was to achieve reliable and repeatable performance while maintaining a simple and modular design while following the game track shown below.
<img width="5139" height="2554" alt="Game_Track" src="https://github.com/user-attachments/assets/98039a66-81f8-45b6-b260-1c250c60be61" />
The game track follows a simple white background with black lines containing 5 checkpoints. The object is to reach all the checkpoints sequentially, while encountering many obstacles such as ping-pong balls, and a parking garage. In order to achieve this objective, the Romi utilizes line following and state estimation, allowing the Romi to understand its position, and aims to follow a consistent path.
## Mechanical Design
...
A particularly challenging aspect of the project was integrating state estimation with the existing line-following control system. While both systems worked independently, combining them introduced unexpected issues where enabling state estimation caused the line-following performance to fail. This required significant debugging and testing to resolve. Ultimately, the issue was addressed, but the process revealed how interconnected subsystems can introduce unintended interactions, especially when sharing data and timing resources. This reinforced the importance of modular design, careful signal management, and incremental integration when developing complex embedded systems. The biggest issue with the state estimation was with the yaw rate. There was an issue where the yaw rate would spike up to 4096 even if the Romi was standing still, causing the heading to be more inconsistent than what it would've normmally been. To combat this, a clamping mechanism was implemented to prevent a yaw rate change of higher than 500. However, the obvious weak link of the system was the heading. If the heading was reliable, the turn towards the garage would've been better, which is where most of the issues lied. For if the heading was off too far to the right, the Romi would be much farther than expected for the length to checkpoint 2, and if it was even farther right, it would hit the edges of the garage.
